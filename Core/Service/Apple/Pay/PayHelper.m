...
#error You need to either convert your project to ARC or add the -fobjc-arc compiler flag to PayHelper.m.
#endif
...
- (id)initWithProductIdentifiers:(NSSet *)productIdentifiers {
    if ((self = [super init])) {
        ...
        for (NSString * productIdentifier in _productIdentifiers) {
            ...
            NSString* password = [SFHFKeychainUtils getPasswordForUsername:productIdentifier andServiceName:@"PayHelper" error:nil];
...
-(BOOL)isPurchasedProductsIdentifier:(NSString*)productID
{
    ...
    NSString* password = [SFHFKeychainUtils getPasswordForUsername:productID andServiceName:@"PayHelper" error:nil];
    if([password isEqualToString:@"YES"])
    {
        productPurchased = YES;
    }
...
- (void)provideContentWithTransaction:(SKPaymentTransaction *)transaction {
    ...
    if (productIdentifier) {
        [SFHFKeychainUtils storeUsername:productIdentifier andPassword:@"YES" forServiceName:@"PayHelper" updateExisting:YES error:nil];
...
- (void)clearSavedPurchasedProductByID:(NSString*)productIdentifier {
    ...
    [SFHFKeychainUtils deleteItemForUsername:productIdentifier andServiceName:@"PayHelper" error:nil];
...
//    NSString* jsonStr = [[NSString alloc] initWithData:jsonData encoding:NSUTF8StringEncoding];
    ...
    if(conn) {
        ...
        error = [NSError errorWithDomain:@"PayHelperError" code:100 userInfo:errorDetail];
